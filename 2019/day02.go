...
func RunIntCodeWithParams(intCode []int, noun int, verb int) int {
	intCode[1] = noun
	intCode[2] = verb
	state := RunIntCodeProgram(intCode)
	return state[0]
}

func FindNounAndVerbFor(intCode []int, expectedOutput int) (int, int) {
	for noun := 0; noun < 100; noun++ {
		for verb := 0; verb < 100; verb++ {
			out := RunIntCodeWithParams(intCode, noun, verb)
			if out == expectedOutput {
				return noun, verb
			}
		}
	}
	panic(fmt.Sprintf("Unable to find noun and verb that produce %v", expectedOutput))
}
...
func Run2019Day02() {
	...
	fmt.Printf("Day 02 : Part 01  Answer:\n\t%d\n", RunIntCodeWithParams(intCode, 12, 2))

	noun, verb := FindNounAndVerbFor(intCode, 19690720)
