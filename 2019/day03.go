...
func FindDistanceToCrossClosestToPort(lines []string) int {
	...
	movementFuncs := map[rune]func(Coord) Coord{
		'U': up,
		'D': down,
		'R': right,
		'L': left,
	...
	//Function for splitting direction instruction, e.g. 'D32',  into a (rune, int)
	splitMovementInstruction := func(s string) (rune, int) {
		var direction rune
		...
		stepping, err := strconv.Atoi(builder.String())
		common.PanicOnError(err)
		return direction, stepping
	}
